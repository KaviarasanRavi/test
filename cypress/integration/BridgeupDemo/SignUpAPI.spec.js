...
1.In package.json you have to change the path of the Script according to the script you are running
  for example :  "cypress run --spec 'cypress/integration/BridgeUpAPIDemo/SignUpAPI.spec.js' --headed"
2.for getting html report you have to use "npm run test" command
3.input data should be available in SignUp.json file under fixtures/API Folder
*/
//const testData = require("../fixtures/ctenant.json")
//var SignUpUsers = require('../../fixtures/signUp.json');
const SignUpUsers = require('../../integration/BridgeupDemo/BridgeUpConfig/signupapi.json');

describe('Company - SignUp - API - Test Suit', () => {
    ...
    SignUpUsers.forEach((user) => {
        ...
        it(`Test:${user.TestId} - ${user.Tenant_Type} - ${user.TestAPIName} API - ${user.TestName} - ${user.TestType}`, () => {
            ...
            if(testid == "TC001"){
                cy.writeFile('cypress/responses/signUpApiResponse.json', 'Company - SignUp - API Validation - Test Suit\n')
            }
            ...
            const  request = cy.request(
                ...
                .then((response) => {
                   ...
                   const resSC = response.status
                    ...
                    cy.writeFile('cypress/responses/signUpApiResponse.json', user.TestId,{ flag: 'a+' })   
                    cy.writeFile('cypress/responses/signUpApiResponse.json', response.body,{ flag: 'a+' })
