...
1.In package.json you have to change the path of the Script according to the script you are running
  for example :  "cypress run --spec 'cypress/integration/BridgeUpAPIDemo/confirmForgotPassword.spec.js' --headed"
2.for getting html report you have to use "npm run test" command
3.input data should be available in confirmForgotPassword.json file under fixtures/API Folder
*/

const SignUpUsers = require('../../fixtures/confirmForgotPassword.json');

describe('Company - confirmForgotPassword - API - Test Suit', () => {
    ...
    SignUpUsers.forEach((user) => {
        ...
        it(`Test:${user.TestId} - ${user.Tenant_Type} - ${user.TestAPIName} API - ${user.TestName} - ${user.TestType}`, () => {
            ...
            if(testid == "TC001"){
                cy.writeFile('cypress/responses/confirmForgotPasswordApiResponse.json', "")
            }

            const  request = cy.request(
                {
                    method: 'POST',
                    url: 'https://auth.devbridgeup.com/confirmForgotPassword',
                ...
                .then((response) => {
                   ...
                   const resSC = response.status
                    ...
                    cy.writeFile('cypress/responses/confirmForgotPasswordApiResponse.json', user.TestId,{ flag: 'a+' })   
                    cy.writeFile('cypress/responses/confirmForgotPasswordApiResponse.json', response.body,{ flag: 'a+' })
