...
1.In package.json you have to change the path of the Script according to the script you are running
  for example :  "cypress run --spec 'cypress/integration/BridgeUpAPIDemo/VerifyEmailAPI.spec.js' --headed"
2.for getting html report you have to use "npm run test" command
3.input data should be available in VerifyEmail.json file under fixtures/API Folder
*/

//const testData = require("../fixtures/ctenant.json")
//var SignUpUsers = require('../../fixtures/signUp.json');
const SignUpUsers = require('../../fixtures/VerifyEmail.json');

describe('Company - SignUpValidation - API - Test Suit', () => {
    ...
    SignUpUsers.forEach((user) => {
        ...
        it(`Test:${user.TestId} - ${user.Tenant_Type} - ${user.TestAPIName} API - ${user.TestName} - ${user.TestType}`, () => {
            ...
            if(testid == "TC001"){
                cy.writeFile('cypress/responses/verifyEmailApiResponse.json', "")
            }

            const  request = cy.request(
                {
                    method: 'POST',
                    url: 'https://auth.devbridgeup.com/verifyEmail',
                ...
                .then((response) => {
                   ...
                   const resSC = response.status
                    ...
                    cy.writeFile('cypress/responses/verifyEmailApiResponse.json', user.TestId,{ flag: 'a+' })   
                    cy.writeFile('cypress/responses/verifyEmailApiResponse.json', response.body,{ flag: 'a+' })
